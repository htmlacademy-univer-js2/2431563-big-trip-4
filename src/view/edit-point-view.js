import { TYPES, EMPTY_POINT, ButtonText } from '../constants.js';
...
function createTypesElement(currentType, isDisabled) {
  const result = TYPES.map((event) => `<div class="event__type-item">
              <input id="event-type-${event.toLowerCase()}-1" class="event__${event.toLowerCase()}-input  visually-hidden" type="radio" name="event-type" value="${event.toLowerCase() }" ${currentType === event ? 'checked' : ''} ${isDisabled ? 'disabled' : ''}>
